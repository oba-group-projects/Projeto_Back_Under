...
import {
  ...
  normalizeOdd,
  moveOddTicks,
  calculateTicksDistance,
} from '../js/core/oddsCalculator.js';
...
assert(
  parelho.lucroMedio === 100,
  `Lay Parelho Lucro Médio: esperado 100, obtido ${parelho.lucroMedio}`
);
...
assert(
  zebra.lucroMedio === 66.6,
  `Lay Zebra Lucro Médio: esperado 66.6, obtido ${zebra.lucroMedio}`
);
assert(
  Math.round(zebra.redMedio) === 200,
  `Lay Zebra Red Médio: esperado ~200, obtido ${zebra.redMedio}`
);
...
assert(
  superZebra.stake === 1110,
  `Lay Super Zebra Stake: esperado 1110, obtido ${superZebra.stake}`
);
assert(
  superZebra.lucroMedio === 66.6,
  `Lay Super Zebra Lucro Médio: esperado 66.6, obtido ${superZebra.lucroMedio}`
);
...
assert(
  drakitoParelho.stake === 800,
  `Drakito Parelho Stake: esperado 800, obtido ${drakitoParelho.stake}`
);
assert(
  drakitoParelho.lucroMedio === 60,
  `Drakito Parelho Lucro Médio: esperado 60, obtido ${drakitoParelho.lucroMedio}`
);
assert(
  drakitoParelho.redMedio === 200,
  `Drakito Parelho Red Médio: esperado 200, obtido ${drakitoParelho.redMedio}`
);
...
assert(
  drakitoFav.lucroMedio === 66.6,
  `Drakito Fav Lucro Médio: esperado 66.6, obtido ${drakitoFav.lucroMedio}`
);
...
assert(
  vovoBack.lucroMedio === 58.38,
  `Vovô Back Lucro Médio: esperado 58.38, obtido ${vovoBack.lucroMedio}`
);
...
assert(
  vovoLay.lucroMedio === 50,
  `Vovô Lay Lucro Médio: esperado 50, obtido ${vovoLay.lucroMedio}`
);
...
assert(getTickSize(1.5) === 0.01, 'Tick size de 1.50 é 0.01');
assert(getTickSize(2.5) === 0.02, 'Tick size de 2.50 é 0.02');
assert(getTickSize(3.5) === 0.05, 'Tick size de 3.50 é 0.05');
assert(getTickSize(5.5) === 0.1, 'Tick size de 5.50 é 0.10');
assert(getTickSize(8.0) === 0.2, 'Tick size de 8.00 é 0.20');

// Mover odds
assert(
  moveOddTicks(3.5, -2) === 3.4,
  `Move 3.50 por -2 ticks (passo 0.05): esperado 3.40, obtido ${moveOddTicks(3.5, -2)}`
);
assert(
  moveOddTicks(2.04, -3) === 1.99,
  `Move 2.04 por -3 ticks cruzando fronteira 2.00: esperado 1.99, obtido ${moveOddTicks(2.04, -3)}`
);

// Distância em ticks
assert(
  calculateTicksDistance(3.5, 3.4) === -2,
  `Distância 3.50 -> 3.40: esperado -2 ticks, obtido ${calculateTicksDistance(3.5, 3.4)}`
);
assert(
  calculateTicksDistance(2.02, 1.98) === -3,
  `Distância 2.02 -> 1.98: esperado -3 ticks, obtido ${calculateTicksDistance(2.02, 1.98)}`
);

console.log('\n--- 3. Testes do Mapa de Pêndulos Parede V6 ---');
const pendulo350 = findClosestPendulo(3.5, '365');
...
const pendulo180 = findClosestPendulo(1.8, '365');
...
const hedgeGreen = calculateHedge({
  oddEntrada: 2.0,
  stakeEntrada: 100,
  oddAtual: 1.9,
  comissaoPct: 0.0325,
...
assert(
  hedgeGreen.lucroLiquido === 5.09,
  `Lucro Líquido: esperado 5.09, obtido ${hedgeGreen.lucroLiquido}`
);
