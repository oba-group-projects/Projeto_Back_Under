 ...
 */
export function calculateHedge({ oddEntrada, stakeEntrada, oddAtual, comissaoPct = 0.0325 }) {
 ...
 */
export function calculateFreebet({ oddEntrada, stakeEntrada, oddAtual, comissaoPct = 0.0325 }) {
  const oddBack = Number(oddEntrada);
  const stakeBack = Number(stakeEntrada);
  const oddLay = Number(oddAtual);
  ...
  const ganhoLiquidoUnder =
    ganhoBrutoUnder > 0 ? ganhoBrutoUnder * (1 - comissaoPct) : ganhoBrutoUnder;
